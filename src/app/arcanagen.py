...
def add_awakenings(loadout, disabled=None):
    ...
    if (
        20 not in disabled
        and len(loadout) >= 1
        and all(grasps.count(g) <= 2 for g in range(1, 7))
    ):
        loadout.append(20)
    if 21 not in disabled and all(c in loadout for c in (16, 17, 22)):
        loadout.append(21)
    if 24 not in disabled and (
        any(all(c in loadout for c in range(r * 5 + 1, r * 5 + 6)) for r in range(4))
        or any(all(c in loadout for c in range(r + 1, r + 22, 5)) for r in range(5))
    ):
        loadout.append(24)
    return loadout
...
def arcana_gen(uid, args):
    if isinstance(args, list) or isinstance(args, tuple):
        ...
        if (
            uid in files.saved_arcana["personal"]
            and args in files.saved_arcana["personal"][uid]
        ):
        ...
        loadout = [
            i
            for i in range(1, 26)
            if bitstring[i - 1] == "1" and all_grasps[i - 1] != 0
        ]
        disabled = set(
            i
            for i in range(1, 26)
            if bitstring[i - 1] == "0" and all_grasps[i - 1] == 0
        )
        add_awakenings(loadout, disabled)

    base = Image.open("/app/resources/arcana/base.png")
    total_grasp = 0
    ...
    for i in range(1, 26):
        if i in loadout:
            ...
            base.paste(Image.open(f"/app/resources/arcana/{i}.png"), (x, y))
