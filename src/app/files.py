...
for god in god_cores:
    with open(
        f'/app/resources/gods/{"".join(god.split())}.txt', "r", encoding="utf8"
    ) as f:
            ...
            *boon, element = boon.split()
            ...
            if boon in boons_info:
                ...
                if _type == "duo":
                    ...
                    aliases["misc"][
                        " ".join(boons_info[boon]["god"][::-1]) + " duo"
                    ] = [boon]
                    aliases["misc"][
                        "duo " + " ".join(boons_info[boon]["god"][::-1])
                    ] = [boon]
                else:
...
with open("/app/resources/aspects.txt", "r", encoding="utf8") as f:
...
for weapon in misc.weapon_icons:
    with open(f"/app/resources/hammers/{weapon}.txt", "r", encoding="utf8") as f:
...
with open("/app/resources/keepsakes.txt", "r", encoding="utf8") as f:
    while keepsake := f.readline().strip():
        ...
        for suffix in ("", " keepsake", "s keepsake", "' keepsake", "'s keepsake"):
            aliases["keepsake"][keepsakes_info[keepsake]["giver"].lower() + suffix] = [
                keepsake
            ]

with open("/app/resources/arcana.txt", "r", encoding="utf8") as f:
    counter = 1
...
with open("/app/resources/vows.txt", "r", encoding="utf8") as f:
    while vow := f.readline().strip():
        vows_info[vow] = {
            "desc": f.readline().strip(),
            "ranks": f.readline().split(),
            "fears": f.readline().split(),
            "flavor": f.readline().strip(),
            "icon": f.readline().strip(),
        }
...
for category in aliases:
    with open(
        f"/app/resources/aliases/{category}aliases.txt", "r", encoding="utf8"
    ) as f:
...
with open("/app/resources/definitions.txt", "r", encoding="utf8") as f:
    aliases["definition"] = {}
...
with open("/app/resources/help.txt", "r", encoding="utf8") as f:
...
with open("/app/resources/enemies.txt", "r", encoding="utf8") as f:
...
def read_personal():
    global saved_arcana
    with open("/app/resources/arcana/loadouts.json", "r", encoding="utf8") as fp:
        saved_arcana = json.loads(fp.read())


def write_personal():
    global saved_arcana
    with open("/app/resources/arcana/loadouts.json", "w", encoding="utf8") as fp:
