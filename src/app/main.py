...
client = commands.Bot(
    command_prefix=["h!", "H!"], case_insensitive=True, intents=intents
)
...
async def on_command_error(ctx, err):
    if isinstance(err, commands.CommandNotFound):
        ...
        if not matches:
            await misc.reply(
                ctx,
                f"h!{err} does not exist, use h!help to see a list of valid commands",
                mention=True,
            )
            return
    ...
    if isinstance(err, commands.MissingRequiredArgument):
        await misc.reply(
            ctx,
            "Missing required input. Run h!help <command_name> for more information.",
            mention=True,
        )
        return
...
async def invite(ctx):
    await misc.reply(
        ctx,
        "https://discordapp.com/api/oauth2/authorize?scope=bot&client_id=1007734213904183306",
    )
...
async def arcanaloadout(ctx, *args):
    ...
    await misc.reply(
        ctx,
        f"Total grasp: **{total_grasp}** <:Grasp:1250935195700563978>",
...
async def react_edit(ctx, embed, choices, embed_function):
    def check(reaction, user):
        return (
            user == ctx.message.author and str(reaction.emoji) in misc.disambig_select
        )
